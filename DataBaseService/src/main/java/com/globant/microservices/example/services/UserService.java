...
public class UserService {
	...
	public UserResource save(final User user) {
		try {
			final User userSaved = userRepository.insert(user);
	...
	public UserResource update(final User user, final String id) throws NotFoundException, ConflictException {
		try {
	...
	public UserResource findById(final String id) throws NotFoundException {
		try {
	...
	public List<UserResource> findByAll() {
		try {
			final List<User> outs = userRepository.findAll();
	...
	public List<UserResource> findByQuery(String value) {
		try {
			final List<User> outs;
			if (StringUtils.isEmpty(value)) {
				outs = userRepository.findAll();
			} else {

				outs = userRepository.findByRegexQuery(value.trim());
			}
			logger.info("Users returned {}", outs.size());
			return outs.stream().map(u -> createResource(u)).collect(Collectors.toList());
		} catch (DataAccessException e) {
			logger.error("Error getting user by query {}, {}", value, e.getMessage(), e);
			throw e;
		}
	}
